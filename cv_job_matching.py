...
class CVJobEmbeddingSimilarity:
    ...
    def generate_embeddings(self, texts: List[str]) -> np.ndarray:
        if self.model_type == "sentence_transformer": return self.get_sentence_transformer_embeddings(texts)
        if self.model_type == "camembert": return self.get_camembert_embeddings(texts)
        if self.model_type == "openai": return self.get_openai_embeddings(texts)
    ...
    def calculate_sectional_similarity(self, cv_data: Dict, job_data: Dict) -> Dict:
        ...
        for section in ['skills','experience','education','global']:
            ...
            if not cv_text or not job_text: similarities[section] = 0.0; continue
            try:
                embeddings = self.generate_embeddings([cv_text, job_text])
                similarities[section] = max(0, cosine_similarity([embeddings[0]], [embeddings[1]])[0][0]) if len(embeddings)==2 else 0.0
            except: similarities[section] = 0.0
        return similarities
    ...
    def calculate_skill_embedding_similarity(self, cv_skills: List[str], job_skills: List[str]) -> Dict:
        ...
        if len(embeddings)!=len(all_skills): return {'average_similarity':0.0,'max_similarity':0.0,'skill_matches':[],'coverage':0.0}
        ...
        for i, job_skill in enumerate(job_skills):
            ...
            skill_matches.append({'job_skill': job_skill,'matched_cv_skill': cv_skills[best_idx],'similarity': float(best_sim)})
            sims.append(best_sim)
        average_sim, max_sim = np.mean(sims), np.max(sims)
        threshold = 0.7
        coverage = sum(1 for s in sims if s>threshold)/len(job_skills)
        return {'average_similarity': float(average_sim),'max_similarity': float(max_sim),
                'skill_matches': skill_matches,'coverage': float(coverage),'similarity_matrix': sim_matrix.tolist()}
    ...
    def calculate_comprehensive_embedding_similarity(self, cv_data: Dict, job_data: Dict) -> Dict:
        ...
        if score_pct>=85: level="Excellente"
        elif score_pct>=70: level="Très bonne"
        elif score_pct>=55: level="Bonne"
        elif score_pct>=40: level="Modérée"
        else: level="Faible"
        return {
