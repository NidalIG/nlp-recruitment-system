...
export default function App(){
  const [cvText,setCvText]=useState(''); const [filename,setFilename]=useState(''); const [jobText,setJobText]=useState(''); const [lastResult,setLastResult]=useState(null);
  return(<div className="min-h-screen"><Header/>
    <div className="mx-auto grid max-w-7xl grid-cols-1 gap-4 p-4 md:grid-cols-[220px_1fr]">
      ...
      <div className="flex flex-col gap-4">
        <UploadSection cvText={cvText} setCvText={setCvText} setFilename={setFilename}/>
        <MatchingSection cvText={cvText} jobText={jobText} setJobText={setJobText} lastResult={lastResult} setLastResult={setLastResult}/>
        <QuickInsights cvText={cvText} jobText={jobText}/>
        <ReportActions cvText={cvText} jobText={jobText} result={lastResult}/>
